...
def upload_short(file_path, title, description, tags):
    ...
    with open(os.path.expanduser("~/credentials.json"), "r") as f:
        data = json.load(f)
        data = data['installed']
    ...
    credentials = service_account.Credentials.from_service_account_file(
        os.path.expanduser(data['auth_uri']), scopes=SCOPE_YOUTUBE, client_id=client_id, client_secret=client_secret
    )
